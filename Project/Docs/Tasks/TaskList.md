...
- [x] **Scene View & マウスピッキング** (2026/05/13): ImGui ウィンドウへの描画対応。
...
- [ ] **Project Browser & アセット管理**: ファイルシステムと連携したドラッグ＆ドロップによるアセット割り当て。
    - [ ] **ProjectView の高度化**:
        - [ ] **ディレクトリ監視 (Directory Watcher)**: OSのイベントを検知し、外部でのファイル変更を即座に反映。
        - [ ] **ビジュアルアイコンの導入**: フォルダやファイル形式に応じたアイコン表示。
        - [ ] **ダブルクリックによる操作**: ディレクトリ移動やアセットの展開（Sceneロード等）を直感的に。
        - [x] ドラッグ＆ドロップの拡張: 
            - [x] Scene View へのドロップによる自動 Entity 生成。 (2026/05/20)
        - [x] **Prefab 編集モード (Prefab Mode)**: Project Browser でのダブルクリックによる専用編集画面への移行と保存機能。 (2026/05/20)
        - [x] **マテリアル保存機能**: Inspector での編集内容を `.mat` ファイルに永続化。 (2026/05/20)
