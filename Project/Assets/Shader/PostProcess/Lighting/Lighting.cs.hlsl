...
void main(uint3 dispatchId : SV_DispatchThreadID) {
	...
	float2 texCoord = float2(dispatchId.x / 1024.0f, dispatchId.y / 1024.0f);
	...
	{	/// lighting
		...
		float specularPow = pow(saturate(NdotH), 1.0f);

		float NdotL = dot(normalize(normal.xyz), -light.direction);
		float cos = pow(NdotL * 0.5f + 0.5f, 2.0f);
		float3 diffuse = (color * light.color * cos * light.intensity).rgb;
		float3 specular = light.color.rgb * light.intensity * specularPow * float3(1.0f, 1.0f, 1.0f);
